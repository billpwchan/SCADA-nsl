...
<c:choose>
<c:when test="${(param['_flowId'] == 'viewAdhocReportFlow' || param['_flowId'] == 'viewReportFlow') && param['decorate'] == 'no'}">
    <%@ include file="withoutDecorator.jsp" %>
</c:when>
<c:when test="${param['_flowId'] != 'viewAdhocReportFlow' && param['decorate'] == 'no'}">
    <%@ include file="emptyDecorator.jsp" %>
</c:when>
<c:otherwise>
